...
const PagamentoConfirmadoEmail = ({ customerName, orderNumber }: Props) => (
  <Html lang="pt-BR" dir="ltr">
    <Head />
    <Preview>Pagamento confirmado — pedido {orderNumber}</Preview>
    <Body style={main}>
      <Container style={container}>
        <Heading style={h1}>{SITE_NAME}</Heading>
        <Hr style={hr} />
        <Heading as="h2" style={h2}>Pagamento Confirmado! ✅</Heading>
        <Text style={text}>Olá {customerName || 'Cliente'},</Text>
        <Text style={text}>O pagamento do seu pedido <strong>{orderNumber}</strong> foi confirmado com sucesso!</Text>
        <Text style={text}>Caso seu pedido necessite de arquivos de arte, envie-os pelo painel do cliente para que possamos iniciar a produção.</Text>
...
export const template = {
  component: PagamentoConfirmadoEmail,
  subject: (data: Record<string, any>) => `Pagamento confirmado — pedido ${data?.orderNumber || ''}`,
