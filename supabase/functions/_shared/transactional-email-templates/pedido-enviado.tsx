...
const PedidoEnviadoEmail = ({ customerName, orderNumber }: Props) => (
  <Html lang="pt-BR" dir="ltr">
    <Head />
    <Preview>Pedido {orderNumber} enviado!</Preview>
    <Body style={main}>
      <Container style={container}>
        <Heading style={h1}>{SITE_NAME}</Heading>
        <Hr style={hr} />
        <Heading as="h2" style={h2}>Pedido Enviado! 📦</Heading>
        <Text style={text}>Olá {customerName || 'Cliente'},</Text>
        <Text style={text}>Seu pedido <strong>{orderNumber}</strong> foi despachado!</Text>
        <Text style={text}>Em breve você receberá mais informações sobre a entrega. Acompanhe pelo painel do cliente.</Text>
...
export const template = {
  ...
  subject: (data: Record<string, any>) => `Pedido ${data?.orderNumber || ''} enviado!`,
